...
@app.get("/")
def read_root():
    return {"Joke categories": ["general", "animal", "food", "math", "programming"], "Usage": ["/jokes", "/jokes/{category}", "/ai_joke", "/ai_joke/{category}"]}
...
def get_ai_joke(category: str):
    if category in config["banned_words"] or (profanity_check and profanity.contains_profanity(category)):
